      ...
      >
        <BaseCard class="d-flex align-items-center text-center h-100 p-2">
          ...
          <h5 class="text-primary">
            {{ $t(`aboutCard.${item.title}.titulo`) }}
          </h5>
          <span class="text-body-secondary">{{
            $t(`aboutCard.${item.title}.descricao`)
          }}</span>
        </BaseCard>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      cardItems: [
        ...
        {
          title: "inteligenciaDados",
          logo: "intelligence",
        },
        {
          title: "nossoPosicionamento",
          logo: "positioning",
        },
        {
          title: "visao",
  ...
  methods: {
    getIcon(name) {
      return this.iconsSvg[`../assets/icons/${name}.svg`];
    },
  },
};
</script>
